...
function gridFactory(direction, justify, alignItems) {
    return (props, childProps, ...children) => re(
        MaterialUI.Grid,
        {container: true, direction, justify, alignItems, ...props},
        React.Children.map(children, child => re(MaterialUI.Grid, {item: true, ...childProps}, child))
    )
}
